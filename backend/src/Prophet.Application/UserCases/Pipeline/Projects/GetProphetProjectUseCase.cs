...
public class GetPipelineProjectUseCase(
    IPipelineProjectStore store,
    IValidator<PipelineProjectIdQuery> validator,
    IValidationErrorCollector errorCollector) : IGetPipelineProjectUseCase
{
    public async Task<PipelineProjectItemDto?> ExecuteAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        var p = await store.GetByIdAsync(id, cancellationToken).ConfigureAwait(false);
        if (p == null)
            return null;
