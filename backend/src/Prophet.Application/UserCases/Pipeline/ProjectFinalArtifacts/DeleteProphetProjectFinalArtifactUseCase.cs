...
public sealed class DeletePipelineFinalArtifactUseCase(
    ...
    IValidationErrorCollector errorCollector) : IDeletePipelineFinalArtifactUseCase
{
    public async Task<bool> ExecuteAsync(Guid projectId, Guid documentId, CancellationToken cancellationToken = default)
    {
        ...
        return await artifactStore.DeleteAsync(projectId, documentId, cancellationToken).ConfigureAwait(false);
    }
}
