...
public sealed class GetPipelineInputDownloadUrlUseCase(
    ...
    IValidationErrorCollector errorCollector) : IGetPipelineInputDownloadUrlUseCase
{
    private static readonly TimeSpan SignedUrlDuration = TimeSpan.FromHours(1);

    public async Task<PipelineInputDownloadDto?> ExecuteAsync(
        Guid projectId,
        Guid documentId,
        CancellationToken cancellationToken = default)
    {
        ...
        return new PipelineInputDownloadDto(url ?? "");
    }
}
